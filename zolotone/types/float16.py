"""IEEE binary16 data-format descriptor."""
...
class Float16(DataType[int]):
    ...
    def to_spec_value(self, raw: int, ctx):
        ...
        self.validate_raw(raw)
        if self.is_inf(raw):
            return fp16.ninf(ctx) if self.sign(raw) else fp16.inf(ctx)
        if self.is_nan(raw):
            return fp16.nan(ctx)
        if self.is_zero(raw):
            return fp16.nzero(ctx) if self.sign(raw) else fp16.zero(ctx)
        return fp16(
