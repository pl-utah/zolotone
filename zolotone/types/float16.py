"""IEEE binary16 descriptor and runtime value."""
...
class Float16(DataType):
    ...
    def from_bits(self, raw: int) -> "Float16Value":
        return Float16Value(self, raw)

    def from_fields(self, sign: int, exponent: int, mantissa: int) -> "Float16Value":
        validate_float_fields(
    ...
    def random_value(self, rng: random.Random) -> "Float16Value":
...
class Float16Value(RuntimeValue):
    dtype: Float16
    raw: int

    def __post_init__(self) -> None:
        if not isinstance(self.dtype, Float16):
            raise TypeError("Float16Value requires a Float16 descriptor")
        validate_float_raw("Float16", self.raw, self.dtype.total_bits())
    ...
    def to_spec(self, ctx):
        from ..spec.custom_specs.fp16 import fp16
        if self.is_inf:
            return fp16.ninf(ctx) if self.sign else fp16.inf(ctx)
        if self.is_nan:
            return fp16.nan(ctx)
        if self.is_zero:
            return fp16.nzero(ctx) if self.sign else fp16.zero(ctx)
        return fp16(
    ...
    def __str__(self) -> str:
        return f"Float16({self.to_python()})"


__all__ = ["Float16", "Float16Value"]
