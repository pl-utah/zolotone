"""BFloat16 data-format descriptor."""
...
class BFloat16(DataType[int]):
    ...
    def from_fields(self, sign: int, mantissa: int, exponent: int) -> Value[int]:
        validate_float_fields(
            type(self).__name__,
            sign,
            exponent,
    ...
    def random_generator(self, seed=None, shared_exponent_bits: int = 0):
        ...
        return generate_bfloat, advance_bfloat
    ...
    def to_spec_value(self, raw: int, ctx):
        ...
        self.validate_raw(raw)
        if self.is_inf(raw):
            return bf16.ninf(ctx) if self.sign(raw) else bf16.inf(ctx)
        if self.is_nan(raw):
            return bf16.nan(ctx)
        if self.is_zero(raw):
            return bf16.nzero(ctx) if self.sign(raw) else bf16.zero(ctx)
        return bf16(
