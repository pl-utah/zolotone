        ...
        try:
            with ProcessPoolExecutor(
                ...
                def submit_until_full() -> None:
                    nonlocal case_stream_exhausted, failed_submission
                    while len(in_flight) < in_flight_limit:
                        try:
                            index, case_ctx = next(indexed_cases)
                        except StopIteration:
                            case_stream_exhausted = True
                            return
                        try:
            ...
            # A killed worker makes this executor unusable. Preserve any
            # results which reached the parent, retry only its unfinished
            # cases serially, then create a fresh pool for the remaining lazy
            # case stream.
            ...
            if not retry_cases:
                if case_stream_exhausted:
                    break
                raise

            print(
                "Verification worker pool terminated abruptly; retrying "
                "unfinished cases serially, then resuming parallel execution.",
                file=sys.stderr,
                flush=True,
            )
