import typing as tp

from ..types import *
from .basics import *
from .Tuple import make_Tuple
from ..ast import *
from ..spec import *


############## Public API ##############

# Function does not care about int_bits/frac_bits types, it takes their values
# Allocates UQ at runtime
def uq_alloc(int_bits: Node,
             frac_bits: Node) -> Op:
    if int_bits.constant is None or frac_bits.constant is None:
        raise TypeError("uq_alloc's arguments must be constant")
    result_dtype = UQ(int_bits.constant.raw, frac_bits.constant.raw)

    def sign(int_bits: UQ, frac_bits: UQ) -> UQ:
        return result_dtype
    
    def impl(int_bits: int, frac_bits: int) -> int:
        return 0
    
    return Op(
        sign=sign,
        impl=impl,
        c_lowering=lambda lowered_args, jittable: "0",
        args=[int_bits, frac_bits],
        name="uq_alloc")


def uq_lt_spec(x: UQ, y: UQ, ctx) -> Bool:
    return x < y


@Primitive(name="uq_lt", spec=uq_lt_spec)
def uq_lt(x: Node, y: Node) -> Node:
    aligned_x, aligned_y = uq_aligner(x, y, None, max)
    return basic_less(aligned_x, aligned_y, out=Bool())


def uq_le_spec(x: UQ, y: UQ, ctx) -> Bool:
    return x <= y


@Primitive(name="uq_le", spec=uq_le_spec)
def uq_le(x: Node, y: Node) -> Node:
    aligned_x, aligned_y = uq_aligner(x, y, None, max)
    return basic_less_or_equal(aligned_x, aligned_y, out=Bool())


def uq_gt_spec(x: UQ, y: UQ, ctx) -> Bool:
    return x > y


@Primitive(name="uq_gt", spec=uq_gt_spec)
def uq_gt(x: Node, y: Node) -> Node:
    aligned_x, aligned_y = uq_aligner(x, y, None, max)
    return basic_greater(aligned_x, aligned_y, out=Bool())


def uq_ge_spec(x: UQ, y: UQ, ctx) -> Bool:
    return x >= y


@Primitive(name="uq_ge", spec=uq_ge_spec)
def uq_ge(x: Node, y: Node) -> Node:
    aligned_x, aligned_y = uq_aligner(x, y, None, max)
    return basic_greater_or_equal(aligned_x, aligned_y, out=Bool())


def uq_eq_spec(x: UQ, y: UQ, ctx) -> Bool:
    return x.eq(y)


@Primitive(name="uq_eq", spec=uq_eq_spec)
def uq_eq(x: Node, y: Node) -> Node:
    aligned_x, aligned_y = uq_aligner(x, y, None, max)
    return basic_equal(aligned_x, aligned_y, out=Bool())


def uq_ne_spec(x: UQ, y: UQ, ctx) -> Bool:
    return x.ne(y)


@Primitive(name="uq_ne", spec=uq_ne_spec)
def uq_ne(x: Node, y: Node) -> Node:
    aligned_x, aligned_y = uq_aligner(x, y, None, max)
    return basic_not_equal(aligned_x, aligned_y, out=Bool())


def uq_aligner(x: Node,
               y: Node,
               int_aggr: tp.Callable | None,
               frac_aggr: tp.Callable | None) -> Node:

    _int_aggr = lambda arg: int_aggr(x.dtype.int_bits, y.dtype.int_bits) if int_aggr is not None else arg.dtype.int_bits
    _frac_aggr = lambda arg: frac_aggr(x.dtype.frac_bits, y.dtype.frac_bits) if frac_aggr is not None else arg.dtype.frac_bits
    
    def uq_aligner_spec(x: UQ, y: UQ, ctx) -> Tuple:
        return x, y

    @Primitive(name="uq_aligner", spec=uq_aligner_spec)
    def impl(x: Node, y: Node) -> Node:
        def align(x):
            int_bits = _int_aggr(x)
            frac_bits = _frac_aggr(x)
            
            shift = frac_bits - x.dtype.frac_bits
            if shift < 0:
                raise NotImplementedError("truncation is not implemented yet")  # truncation
            
            # frac bits extension
            if shift > 0:
                return basic_lshift(
                    x,
                    Const(UQ.from_int(shift)),
                    UQ(int_bits, frac_bits),
                )

            # no extension
            if int_bits == x.dtype.int_bits and frac_bits == x.dtype.frac_bits:
                return x
            
            # int bits extension
            return basic_identity(x, UQ(int_bits, frac_bits))
        return make_Tuple(align(x), align(y))
    
    return impl(x, y)


def uq_frac_bits(x: Node) -> Node:
    if not isinstance(x.dtype, UQ):
        raise TypeError(f"Expected UQ node, got {type(x.dtype).__name__}")
    return Const(UQ.from_int(x.dtype.frac_bits))


def uq_int_bits(x: Node) -> Node:
    if not isinstance(x.dtype, UQ):
        raise TypeError(f"Expected UQ node, got {type(x.dtype).__name__}")
    return Const(UQ.from_int(x.dtype.int_bits))


def uq_fraction_to_integer(x: Node) -> Primitive:
    """Reinterpret a UQ fractional field as an integer field."""

    if x.dtype.int_bits != 0:
        raise ValueError(
            "uq_fraction_to_integer expects a UQ value with zero integer bits"
        )
    frac_bits = x.dtype.frac_bits

    def spec(x: UQ, ctx) -> UQ:
        return x * ctx.two() ** ctx.real_val(frac_bits)

    @Primitive(name="uq_fraction_to_integer", spec=spec, c_inline=True)
    def impl(x: Node):
        return basic_identity(x=x, out=UQ(frac_bits, 0))

    return impl(x)


def uq_integer_to_fraction(x: Node) -> Primitive:
    """Reinterpret a UQ integer field as a fractional field."""

    if x.dtype.frac_bits != 0:
        raise ValueError(
            "uq_integer_to_fraction expects a UQ value with zero fractional bits"
        )
    int_bits = x.dtype.int_bits

    def spec(x: UQ, ctx) -> UQ:
        return x * ctx.two() ** (-ctx.real_val(int_bits))

    @Primitive(name="uq_integer_to_fraction", spec=spec, c_inline=True)
    def impl(x: Node):
        return basic_identity(x=x, out=UQ(0, int_bits))

    return impl(x)


def uq_zero_extend(x: Node, n: int) -> Node:
    if not isinstance(n, int):
        raise TypeError(f"n must be int, given: {type(n).__name__}")
    if n < 0:
        raise ValueError(f"n must be non-negative, given: {n}")
    
    def uq_zero_extend_spec(x: UQ, ctx) -> UQ:
        return x

    @Primitive(name="uq_zero_extend", spec=uq_zero_extend_spec)
    def impl(x: Node) -> Node:
        return basic_identity(
            x=x,
            out=UQ(x.dtype.int_bits + n, x.dtype.frac_bits),
        )
    
    return impl(x)


def uq_add_spec(x: UQ, y: UQ, ctx) -> UQ:
    return x + y


@Primitive(name="uq_add", spec=uq_add_spec)
def uq_add(x: Node, y: Node) -> Node:
    target_int_bits = max(x.dtype.int_bits, y.dtype.int_bits) + 1
    target_frac_bits = max(x.dtype.frac_bits, y.dtype.frac_bits)
    
    x_adj, y_adj = uq_aligner(
        x=x,
        y=y,
        int_aggr=None,
        frac_aggr=lambda x, y: max(x, y),
    )
    root = basic_add(
        x=x_adj,
        y=y_adj,
        out=UQ(target_int_bits, target_frac_bits),
    )
    return root


def uq_sub_spec(x: UQ, y: UQ, ctx) -> UQ:
    return x - y


@Primitive(name="uq_sub", spec=uq_sub_spec)
def uq_sub(x: Node, y: Node) -> Node:
    target_int_bits = max(x.dtype.int_bits, y.dtype.int_bits) + 1
    target_frac_bits = max(x.dtype.frac_bits, y.dtype.frac_bits)
    
    x_adj, y_adj = uq_aligner(
        x=x,
        y=y,
        int_aggr=None,
        frac_aggr=lambda x, y: max(x, y),
    )
    root = basic_sub(
        x=x_adj, 
        y=y_adj,
        out=UQ(target_int_bits, target_frac_bits),
    )
    return root


def uq_max_spec(x: UQ, y: UQ, ctx) -> UQ:
    return x.max(y)


@Primitive(name="uq_max", spec=uq_max_spec)
def uq_max(x: Node, y: Node) -> Node:
    target_int_bits = max(x.dtype.int_bits, y.dtype.int_bits)
    target_frac_bits = max(x.dtype.frac_bits, y.dtype.frac_bits)
    
    x_adj, y_adj = uq_aligner(
        x=x, 
        y=y, 
        int_aggr=None, 
        frac_aggr=lambda x, y: max(x, y),
    )
    root = basic_max(
        x=x_adj, 
        y=y_adj,
        out=UQ(target_int_bits, target_frac_bits),
    )
    return root


def uq_min_spec(x: UQ, y: UQ, ctx) -> UQ:
    return x.min(y)


@Primitive(name="uq_min", spec=uq_min_spec)
def uq_min(x: Node, y: Node) -> Node:
    target_int_bits = max(x.dtype.int_bits, y.dtype.int_bits)
    target_frac_bits = max(x.dtype.frac_bits, y.dtype.frac_bits)
    
    x_adj, y_adj = uq_aligner(
        x=x,
        y=y,
        int_aggr=None, 
        frac_aggr=lambda x, y: max(x, y),
    )
    root = basic_min(
        x=x_adj, 
        y=y_adj,
        out=UQ(target_int_bits, target_frac_bits),
    )
    return root


def uq_mul_spec(x: UQ, y: UQ, ctx) -> UQ:
    return x * y


@Primitive(name="uq_mul", spec=uq_mul_spec)
def uq_mul(x: Node, y: Node) -> Node:
    target_int_bits = x.dtype.int_bits + y.dtype.int_bits
    target_frac_bits = x.dtype.frac_bits + y.dtype.frac_bits
    
    root = basic_mul(
        x=x,
        y=y,
        out=UQ(target_int_bits, target_frac_bits),
    )
    return root


def uq_to_q_spec(x: UQ, ctx) -> Q:
    return x


@Primitive(name="uq_to_q", spec=uq_to_q_spec, c_inline=True)
def uq_to_q(x: Node) -> Node:
    return basic_identity(
        x=x,
        out=Q(x.dtype.int_bits + 1, x.dtype.frac_bits),
    )


def uq_rshift_spec(x: UQ, amount: DataType, ctx) -> UQ:
    return x * (ctx.two() ** (-amount))


@Primitive(name="uq_rshift", spec=uq_rshift_spec)
def uq_rshift(x: Node, amount: Node) -> Node:
    return basic_rshift(
        x=x,
        amount=amount,
        out=x.dtype,
    )


def uq_rshift_jam_spec(x: UQ, amount: DataType, ctx) -> UQ:
    return x * (ctx.two() ** (-amount))


@Primitive(name="uq_rshift_jam", spec=uq_rshift_jam_spec)
def uq_rshift_jam(x: Node, amount: Node) -> Node:
    one = Const(
        UQ(x.dtype.int_bits, x.dtype.frac_bits).from_bits(1),
    )
    shifted_bit_mask = basic_sub(
        basic_lshift(
            one,
            amount,
            out=x.dtype,
        ),
        one,
        out=x.dtype,
    )
    sticky_masked_bits = basic_and(x, shifted_bit_mask, out=x.dtype)
    sticky_bit = basic_or_reduce(sticky_masked_bits, UQ(1, 0))
    return basic_or(uq_rshift(x, amount), sticky_bit, x.dtype)


# TODO: truncation
def uq_lshift_spec(x: UQ, amount: DataType, ctx) -> UQ:
    return x * (ctx.two() ** amount)


@Primitive(name="uq_lshift", spec=uq_lshift_spec)
def uq_lshift(x: Node, amount: Node) -> Node:
    root = basic_lshift(
        x=x,
        amount=amount,
        out=x.dtype,
    )
    return root


def uq_select(x: Node, start: int, end: int) -> Node:
    width = start - end + 1
    x_frac_bits = x.dtype.frac_bits
    
    frac_bits = max(0, min(start, x_frac_bits - 1) - end + 1) if x_frac_bits > 0 else 0
    int_bits = width - frac_bits
    
    def spec(x: UQ, ctx) -> UQ:
        slice1 = ctx.fresh_real("slice1")
        slice2 = ctx.fresh_real("slice2")
        ctx.assume(x.eq(slice1 + slice2))
        return slice1
    
    @Primitive(name="uq_select", spec=spec, c_inline=True)
    def impl(x: Node) -> Node:
        out = UQ(int_bits, frac_bits)
        root = basic_select(x, start, end, out)
        return root
    
    return impl(x)


def uq_split(x: Node, idx: int) -> Node:
    # Returns Tuple(lo, hi), where lo are the lowest `idx` bits.
    if not isinstance(idx, int):
        raise TypeError(f"idx must be int, given: {idx}")
    total_bits = x.dtype.total_bits()
    if idx <= 0 or idx >= total_bits:
        raise ValueError(f"idx must be in (0, {total_bits}), given: {idx}")
    
    x_int_bits = x.dtype.int_bits
    x_frac_bits = x.dtype.frac_bits
    
    lo_width = idx
    lo_frac_bits = min(x_frac_bits, lo_width)
    lo_int_bits = lo_width - lo_frac_bits
    
    hi_width = total_bits - lo_width
    hi_frac_bits = x_frac_bits - lo_frac_bits
    hi_int_bits = hi_width - hi_frac_bits
    
    def spec(x: UQ, ctx) -> Tuple:
        lo = ctx.fresh_real("lo")
        hi = ctx.fresh_real("hi")
        
        two = ctx.two()
        zero = ctx.zero()
        
        # x = hi * 2^lo_int_bits + lo * 2^-hi_frac_bits
        ctx.assume(
            x.eq(
                (hi * two ** ctx.real_val(lo_int_bits))
                + (lo * two ** (-ctx.real_val(hi_frac_bits)))
            )
        )
        # Preserve the exact zero behavior explicitly. Although lo and hi are
        # unsigned, the simplifier does not always derive both equalities from
        # the decomposition equation when x has simplified to zero.
        ctx.assume(
            (~x.eq(zero))
            | (lo.eq(zero) & hi.eq(zero))
        )
        return (lo, hi)
    
    @Primitive(name="uq_split", spec=spec)
    def impl(x: Node) -> Node:
        lo = basic_select(x, idx - 1, 0, UQ(lo_int_bits, lo_frac_bits))
        
        hi = basic_select(x, total_bits - 1, idx, UQ(hi_int_bits, hi_frac_bits))
        
        return make_Tuple(lo, hi)
    
    return impl(x)


def uq_resize(x: Node, int_bits: int, frac_bits: int) -> Node:
    if not isinstance(int_bits, int) or not isinstance(frac_bits, int):
        raise TypeError("int_bits and frac_bits must be integers")
    if int_bits < 0 or frac_bits < 0:
        raise ValueError("int_bits and frac_bits must be non-negative")

    frac_shift = frac_bits - x.dtype.frac_bits
    
    def uq_resize_spec(x: UQ, ctx) -> UQ:
        return x

    @Primitive(name="uq_resize", spec=uq_resize_spec)
    def impl(x: Node) -> Node:
        out = UQ(int_bits, frac_bits)
        if frac_shift > 0:
            return basic_lshift(
                x=x,
                amount=Const(UQ.from_int(frac_shift)),
                out=out,
            )
        if frac_shift < 0:
            return basic_rshift(
                x=x,
                amount=Const(UQ.from_int(-frac_shift)),
                out=out,
            )
        return basic_identity(x=x, out=out)
    return impl(x)


def uq_is_zero_spec(x: UQ, ctx) -> UQ:
    return x.eq(ctx.zero())


@Primitive(name="uq_is_zero", spec=uq_is_zero_spec)
def uq_is_zero(x: Node) -> Node:
    return basic_invert(basic_or_reduce(x, UQ(1, 0)), UQ(1, 0))
