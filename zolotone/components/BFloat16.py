...
def bf16_decode(x: Node) -> DecodedBF16:
    ...
    def decode(x: Node) -> Node:
        ...
        mantissa_is_nonzero = basic_or_reduce(mantissa, out=UQ(1, 0))
        mantissa_is_zero = basic_invert(mantissa_is_nonzero, out=UQ(1, 0))

        exponent_is_all_ones = basic_and_reduce(exponent, out=UQ(1, 0))
        exponent_is_not_all_ones = basic_invert(exponent_is_all_ones, out=UQ(1, 0))
        exponent_is_nonzero = basic_or_reduce(exponent, out=UQ(1, 0))
        exponent_is_zero = basic_invert(exponent_is_nonzero, out=UQ(1, 0))

        is_normal = basic_and(exponent_is_nonzero, exponent_is_not_all_ones, UQ(1, 0))
        is_subnormal = basic_and(exponent_is_zero, mantissa_is_nonzero, UQ(1, 0))
        is_zero = basic_and(exponent_is_zero, mantissa_is_zero, UQ(1, 0))
        is_inf = basic_and(exponent_is_all_ones, mantissa_is_zero, UQ(1, 0))
        is_nan = basic_and(exponent_is_all_ones, mantissa_is_nonzero, UQ(1, 0))
