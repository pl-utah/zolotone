...
def effective_exponent(value) -> Node:
    ...
    return if_then_else(
        value.is_sub,
        Const(UQ(exponent_type.int_bits, exponent_type.frac_bits).from_bits(1)),
        value.exponent,
    )


def and_spec(x: UQ(1, 0), y: UQ(1, 0), ctx) -> UQ(1, 0):
    return If(
...
def xor_spec(x: UQ(1, 0), y: UQ(1, 0), ctx) -> UQ(1, 0):
...
def or_spec(x: UQ(1, 0), y: UQ(1, 0), ctx) -> UQ(1, 0):
    return If(
        x.eq(ctx.one()) | y.eq(ctx.one()),
        ctx.one(),
        ctx.zero(),
...
def neg_spec(x: UQ(1, 0), ctx) -> UQ(1, 0):
    return If(
        x.eq(ctx.one()),
        ctx.zero(),
        ctx.one(),
