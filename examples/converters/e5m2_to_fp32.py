...
def spec_e5m2_to_fp32(x: E5M2, ctx) -> Float32:
    return Cases(
...
if __name__ == "__main__":
    cast = e5m2_to_fp32(
        Var(name="x", dtype=E5M2()),
    )

    cast.check_determinism()
    cast.check_spec()

    with open("examples/c_models/e5m2_to_fp32_jit.cpp", "w") as file:
        file.write(cast.to_cpp(jittable=True))

    with open("examples/c_models/e5m2_to_fp32_no_jit.cpp", "w") as file:
