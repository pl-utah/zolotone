from zolotone import *


def spec_fp32_to_fp16(x: Float32, ctx) -> Float16:
    return Cases(
        case(x.is_nan, fp16.nan(ctx)),
        case(x.is_ninf, fp16.ninf(ctx)),
        case(x.is_pinf, fp16.inf(ctx)),
        case(x.is_nzero, fp16.nzero(ctx)),
        case(x.is_finite, fp16.encode(x.value, ctx)),
        ctx=ctx,
    )


@Composite(name="fp32_to_fp16", spec=spec_fp32_to_fp16)
def fp32_to_fp16(x: Node) -> Node:
    X = fp32_decode(x)

    significand = effective_significand(X)
    significand = uq_resize(
        significand,
        1,
        max(Float32.mantissa_bits, Float16.mantissa_bits),
    )

    source_exponent = effective_exponent(X)
    target_exponent = q_add(
        uq_to_q(source_exponent),
        Const(Q.from_int(Float16.exponent_bias - Float32.exponent_bias)),
    )

    result = fp16_encode(X.sign, target_exponent, significand)

    encode_negative_zero = bit_and(X.is_zero, X.sign)
    result = if_then_else(
        encode_negative_zero,
        Const(Float16().nZero()),
        result,
    )

    encode_positive_infinity = bit_and(X.is_inf, bit_neg(X.sign))
    result = if_then_else(
        encode_positive_infinity,
        Const(Float16().Inf()),
        result,
    )

    encode_negative_infinity = bit_and(X.is_inf, X.sign)
    result = if_then_else(
        encode_negative_infinity,
        Const(Float16().nInf()),
        result,
    )

    result = if_then_else(
        X.is_nan,
        Const(Float16().NaN()),
        result,
    )
    return result


if __name__ == "__main__":
    cast = fp32_to_fp16(
        Var(name="x", dtype=Float32()),
    )

    cast.check_determinism()
    cast.check_spec()

    with open("examples/c_models/fp32_to_fp16_jit.cpp", "w") as file:
        file.write(cast.to_cpp(jittable=True))

    with open("examples/c_models/fp32_to_fp16_no_jit.cpp", "w") as file:
        file.write(cast.to_cpp(jittable=False))
