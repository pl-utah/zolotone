...
def wgmma_fp16_e4m3_e5m2(
    ...
    product_exponent = if_then_else(
        ...
        Const(
            UQ(
                maximum_product_exponent.dtype.int_bits,
                maximum_product_exponent.dtype.frac_bits,
            ).from_bits(0),
        ),
        maximum_product_exponent,
    )
    ...
    c_exponent = if_then_else(
        C.is_zero,
        Const(UQ(c_exponent.dtype.int_bits, 0).from_bits(0)),
        c_exponent,
    )
...
if __name__ == "__main__":
    ...
    for jittable, suffix in ((True, "jit"), (False, "no_jit")):
        (output_directory / f"wgmma_fp16_e4m3_e5m2_{suffix}.cpp").write_text(
