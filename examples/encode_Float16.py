...
def fp16_encodings(m_rounded_uq: Node, e_rounded_uq: Node):
    ...
    final_e_uq = basic_identity(
        x=final_e_uq_wide,
        out=Const(UQ.from_int(Float16.inf_code)),
    )
...
def fp16_encode_spec(s, e, m, ctx):
    ...
    finite_value = (
        ...
        * (
            ctx.two()
            ** (e - ctx.real_val(Float16.exponent_bias))
        )
    )
...
def fp16_encode(s_uq: Node, e_q: Node, m_uq: Node) -> Node:
    ...
    final_m_uq, final_e_uq = fp16_encodings(
        m_rounded_uq,
        e_rounded_uq,
    )

    packed_fp16 = fp16_pack(s_uq, final_e_uq, final_m_uq)
    return if_then_else(
        encode_exact_zero,
        Const(Float16.Zero()),
        packed_fp16,
